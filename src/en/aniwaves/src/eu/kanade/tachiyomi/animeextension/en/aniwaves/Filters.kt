...
object Filters {
    ...
    internal open class CheckBoxFilterList(
        ...
        fun selectedValues() = state.mapIndexedNotNull { index, box ->
            values.getOrNull(index)?.second?.takeIf { box.state }
        }
    }
...
internal fun AnimeFilterList.getSearchParameters(): Filters.FilterSearchParams {
    ...
    return Filters.FilterSearchParams(
        ...
        genres = firstInstanceOrNull<Filters.GenreFilter>()?.selectedValues().orEmpty(),
        countries = firstInstanceOrNull<Filters.CountryFilter>()?.selectedValues().orEmpty(),
        seasons = firstInstanceOrNull<Filters.SeasonFilter>()?.selectedValues().orEmpty(),
        years = firstInstanceOrNull<Filters.YearFilter>()?.selectedValues().orEmpty(),
        types = firstInstanceOrNull<Filters.TypeFilter>()?.selectedValues().orEmpty(),
        statuses = firstInstanceOrNull<Filters.StatusFilter>()?.selectedValues().orEmpty(),
        language = firstInstance<Filters.LanguageFilter>().toQueryPart(),
        ratings = firstInstanceOrNull<Filters.RatingFilter>()?.selectedValues().orEmpty(),
    )
}
