...
class AniWaves :
    ...
    override fun searchAnimeRequest(page: Int, query: String, filters: AnimeFilterList): Request {
        ...
        if (normalizedQuery.startsWith("#")) {
            val slug = normalizedQuery.removePrefix("#").trim()
                .lowercase(Locale.US)
                .replace(" ", "-")
        ...
        val url = "$baseUrl/filter".toHttpUrl().newBuilder().apply {
            if (normalizedQuery.isNotBlank()) addQueryParameter("keyword", normalizedQuery)
    ...
    private fun Document.resolveIfSearch(): Document {
        val pageUrl = location().toHttpUrlOrNull() ?: return this
        if (pageUrl.encodedPath != "/filter" || pageUrl.queryParameter("keyword").isNullOrBlank()) return this
