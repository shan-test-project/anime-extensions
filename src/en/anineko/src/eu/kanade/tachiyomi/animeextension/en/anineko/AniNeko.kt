...
class AniNeko :
    ...
    override fun videoListParse(response: Response): List<Video> {
        ...
        val parsedVideos = filteredVideos.map { video ->
            video to parseVideoInfo(video.quality)
        }
        val availableQualities = parsedVideos.mapNotNull { (_, info) -> info.quality }.toSet()
        val targetQuality = when {
            preferredQuality == PREF_QUALITY_AUTO -> qualityOrder.firstOrNull { it in availableQualities }
            preferredQuality in availableQualities -> preferredQuality
            else -> "1080p"
        }
        ...
        return parsedVideos
            .sortedWith(
                compareBy<Pair<Video, VideoInfo>>(
                    ...
                    { (_, info) -> if (info.quality == targetQuality) 0 else 1 },
                    { (_, info) ->
    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) {
        screen.addListPreference(
            ...
            entryValues = listOf(PREF_QUALITY_AUTO, "1080p", "720p", "480p", "360p"),
    ...
    companion object {
        ...
        private const val PREF_QUALITY_AUTO = "Auto"
